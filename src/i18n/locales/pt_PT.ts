import translation from "../constant-keys";

export default {
  // A
  [translation.ACCUMULATE_VULTIES_PERIOD]: 
    "Os seus ativos acumularão VULTIES durante um período de 12 meses. No final deste período, receberá a sua parte do airdrop (5% de $VULT).",
  [translation.ACTIVE_POSITIONS]: "Posições ativas",
  [translation.ADD_NEW_VAULT]: "Adicionar novo cofre",
  [translation.AIRDROP_CALCULATION_NOTE]: 
    "Nota: O airdrop é baseado no valor total no seu cofre multiplicado pelo tempo em que os ativos são mantidos no cofre. Quantias maiores durante mais tempo ganham mais.",
  [translation.AIRDROP_LEADERBOARD]: "Classificação do airdrop",
  [translation.AIRDROP_NOT_STARTED]: "O airdrop ainda não começou!",
  [translation.AIRDROP_SHARE_CALCULATION]: 
    "A sua parte do airdrop é baseada na quantidade de ativos em Vultisig multiplicada pelo tempo no cofre.",
  [translation.ASSENT_COUNT_MANAGE]: "Os ativos que contam para o airdrop podem ser",
  // B
  [translation.BALANCES]: "Saldos",
  [translation.BECOME_VULTISIG_USER]: "Torne-se um utilizador Vultisig e participe no cálculo do airdrop.",
  [translation.BOND_TITLE]: "Obrigações",
  // C
  [translation.CHECK_LEANDER_BOARD]: "Ver classificação",
  [translation.CHOOSE_CHAIN]: "Escolher cadeias",
  [translation.CHROME_EXTENSION]: "Extensão Chrome",
  [translation.CHOOSE_TOKEN]: "Escolher tokens",
  [translation.CONGRATULATIONS]: "Parabéns!",
  [translation.CONNECT_OR_UPLOAD]: "Conecte-se com VultiConnect ou carregue o seu código QR do cofre para começar",
  [translation.CONNECT_YOUR_WALLET]: "Conecte a sua carteira",
  [translation.COUNTED]: "contado",
  [translation.CURRENCY]: "Moeda",
  // D
  [translation.DAILY_UPDATE_POINTS]: "Os pontos e saldos são sempre atualizados no final do dia",
  [translation.DOWNLOAD_APP]: "Ainda não tem um cofre? Baixe o Vultisig agora",
  [translation.DOWNLOAD_APP_DESC_ONBOARDING]: 
    "Comece por baixar a aplicação Vultisig da App Store ou Google Play para aceder à plataforma e ao airdrop.",
  [translation.DOWNLOAD_APP_TITLE_ONBOARDING]: "DESCARREGAR A APLICAÇÃO",
  [translation.DEFAULT_CHAINS]: "Cadeias padrão",
  [translation.DELETE_VAULT_WARNING]: "Está a remover o cofre",
  [translation.DROP_FILE_HERE]: "Arraste o seu ficheiro para aqui ou",
  // E
  [translation.EDIT_VAULT]: "Editar o nome do seu cofre",
  [translation.ERROR_EXISTS_IMPORT]: "O cofre já existe",
  [translation.ERROR_NOT_FOUND_IMPORT]: "Nenhum cofre encontrado",
  [translation.ERROR_FILE_EXTENSION]: "Extensão de ficheiro inválida",
  [translation.ERROR_INVALID_FILE]: "Ficheiro inválido",
  [translation.ETH_ARB_ADDRESS_NOT_FOUND]: "Endereço ETH/ARB não encontrado no seu cofre",
  // F
  [translation.FARMED]: "CULTIVADO",
  [translation.FAQ]: "Perguntas frequentes",
  [translation.FILE_SELECTED_SUCCESS]: "Ficheiro selecionado com sucesso",
  // G
  // H
  [translation.HERE_LINK]: "aqui",
  [translation.HINT]:
    "Se ainda não guardou o código QR, pode encontrá-lo na aplicação no canto superior direito do ecrã principal",
  [translation.HOW_TO_PARTICIPATE]: "Como participar",
  // I
  [translation.INSPECT_CODE_MANAGE]: "Inspecione o código aqui.",
  // J
  [translation.JOIN_AIRDROP]: "Participar no airdrop",
  [translation.JOIN_THE_AIRDROP_ONBOARDING]: "PARTICIPE NO AIRDROP",
  [translation.JOIN_THE_AIRDROP_ONBOARDING_EXPLAIN]:
    "Participe no airdrop e siga a classificação em AIRDROP.VULTISIG.COM",
  // K
  // L
  [translation.LANGUAGE]: "Idioma",
  [translation.LIQUIDITY_POSITION_TITLE]: "Posição de liquidez",
  [translation.LINK_SEE_HERE]: "ver aqui.",
  [translation.LOADING]: "A carregar",
  [translation.LOGO]: "Logo",
  [translation.LOGO_SIZE]: "O tamanho máximo permitido do logotipo é 100 KB",
  [translation.LOGOUT]: "Terminar sessão",
  [translation.LOGOUT_VAULT]: "Termine a sessão do cofre na web",
  [translation.LOGOUT_VAULT_TITLE]: "Terminar sessão do cofre",
  [translation.LOGOUT_VAULT_WARNING]: "Está a terminar a sessão do cofre",
  // M
  [translation.MANAGE_AIRDROP]: "Gerir o airdrop",
  [translation.MAYA]: "Maya",
  // N
  [translation.NO_BOND_FOUND]: "Nenhuma obrigação encontrada",
  [translation.NOT_COLLECTED]: "Nenhuma outra informação recolhida.",
  [translation.NO_LIQUIDITY_POSITION]: "Nenhuma posição de liquidez encontrada",
  [translation.NO_LP_POSITION]: "Nenhuma posição LP encontrada",
  [translation.NO_PROVIDER_POSITION]: "Nenhuma posição de fornecedor encontrada",
  [translation.NO_SAVER_POSITION]: "Nenhuma posição de poupador encontrada",
  // O
  [translation.OTHER]: "Outro",
  // P
  [translation.POWERED_BY]: "Fornecido por",
  [translation.POSITIONS_TITLE]: "Posições",
  [translation.PRIVACY]: "Leia o compromisso de privacidade do fundador aqui.",
  // Q
  [translation.QUESTION_FIRST_ONBOARDING]: "Como me registro para o airdrop?",
  [translation.QUESTION_SECOND_ONBOARDING]: "Que ativos contam para o airdrop?",
  [translation.QUESTION_THIRD_ONBOARDING]: "Qual é o processo do airdrop?",
  // R
  [translation.READ_MORE_ABOUT_IT]: "Leia mais sobre isso",
  [translation.REFER_FRIEND_ONBOARDING]: "INDIQUE UM AMIGO",
  [translation.REFER_FRIEND_ONBOARDING_EXPLAIN]:
    "Indique amigos com a nossa simples Mini App do Telegram e aumente o seu airdrop",
  [translation.REGISTER_PUBLIC_KEYS_VAULTS]: "Está a registar as suas chaves públicas e os endereços dos cofres.",
  [translation.REGISTER_UNLIMITED_VAULTS]: "Pode registar quantos cofres quiser.",
  [translation.REMOVE_VAULT]: "Remova o seu cofre da web",
  [translation.REMOVE_VAULT_TITLE]: "Remover cofre",
  [translation.REMOVE]: "Remover",
  [translation.RENAME_VAULT]: "Renomear cofre",
  [translation.RENAME]: "Renomear",
  [translation.REPEAT_REGISTRATION]: "Pode se registrar quantas vezes quiser.",
  [translation.RUNE_PROVIDER_TITLE]: "Fornecedor de Rune",
  // S
  [translation.SAVE]: "Guardar",
  [translation.SAVER_TITLE]: "Poupador",
  [translation.SEARCH]: "Pesquisar",
  [translation.SECURE_CRYPTO_VAULT]: "COFRE CRIPTO SEGURO",
  [translation.SEE_FULL_LIST]: "Ver lista completa",
  [translation.SELECT_CHAINS_TOKENS]:
    "Nesta página, escolha todas as cadeias e tokens que deseja ativar para que sejam contados para o airdrop.",
  [translation.SETUP_MULTIـONBOARDING]: "CONFIGURAR WALLET MULTIFATOR",
  [translation.SETUP_MULTI_ONBOARDING_EXPLAIN]: 
    "Crie uma wallet segura com autenticação multifator para proteger os seus ativos e melhorar a segurança da sua conta.",
  [translation.SHARE_VAULT]: "Partilhe o seu cofre",
  [translation.SHARE_SETTINGS_TITLE]: "Personalize a página do cofre partilhado",
  [translation.SHARE_SETTINGS]: "Personalize a aparência do cofre partilhado",
  [translation.SHARE_VAULT_THEME]: "Tema do cofre partilhado",
  [translation.SHARE_VAULT_LOGO]: "Logotipo do cofre partilhado",
  [translation.SHARE_CURRENT_LOGO]: "O seu logotipo atual",
  [translation.SHARE_UPLOAD_LOGO]: "Carregue o seu logotipo",
  [translation.SOMETHING_WRONG]: "Algo correu mal",
  [translation.STAKE_TITLE]: "Stake",
  [translation.START]: "Iniciar",
  [translation.SUCCESSFUL_COPY_LINK]: "Link copiado para a área de transferência",
  [translation.SUCCESSFUL_COPY_TEXT]: "Texto copiado para a área de transferência",
  [translation.SUPPORTED_ASSETS_NOTE]: 
    "Ativos de nível 1 e a maioria dos tokens suportados por Vultisig são contados. Outros ativos ativos, como LPs e obrigações de nodes da THORChain, Protocolo MAYA e tokens em staking, também são válidos.",
  // T
  [translation.TGT]: "TGT",
  [translation.TITLE_ONBOARDING]: "como participar no AIRDROP DE <span>$5M</span> DA VULTISIG",
  [translation.TOTAL_AIRDROP_VAULT_VALUE]: "Valor total dos cofres do airdrop",
  [translation.TOTAL_BALANCE]: "Saldo total",
  [translation.TOTAL_REGISTERED_WALLETS]: "Total de wallets registadas",
  [translation.THORCHAIN]: "Thorchain",
  [translation.TRANSFER_FUNDS_ONBOARDING]: "TRANSFERIR FUNDOS PARA COFRES",
  [translation.TRANSFER_FUNDSـONBOARDING_EXPLAIN]: 
    "Deposite fundos nos cofres de Vultisig para se qualificar para o airdrop e participar nas atividades da plataforma.",

  // U
  [translation.UNSUCCESSFUL_COPY_LINK]: "Falha ao copiar o link",
  [translation.UNSUCCESSFUL_COPY_TEXT]: "Falha ao copiar o texto",
  [translation.UNREGISTER_VAULT_INFO]: 
    "Cancelar o registo de um cofre remove-o da classificação. Pode levar até um dia para que o saldo seja refletido novamente.",
  [translation.UPLOAD_IT]: "carregue-o",
  [translation.UPLOAD_QR_CODE]: "Carregue o seu código QR aqui",
  [translation.UPLOAD_VAULT_SHARE]: "Carregue a sua chave pública Vultisig para começar",
  [translation.UPLOAD_VAULT_QR]: "Carregue o QR do cofre",
  // V
  [translation.VULTICONNECT]: "VultiConnect",
  [translation.VAULT_AIRDROP_JOINED]: 
    "O seu cofre {0} juntou-se ao airdrop da Vultisig. Agora começará a acumular VULTIES diariamente. Volte amanhã para ver os seus primeiros VULTIES na classificação e verificar os seus concorrentes.",
  [translation.VAULT_SETTINGS]: "Configurações do cofre",
  [translation.VULT_TOKEN]: "$VULT Token",
  [translation.VULTISIG]: "Vultisig",
  // W
  [translation.WEWE]: "WEWE",
  [translation.WEWE_USDT_ADDRESS_NOT_FOUND]: "Endereço WEWE/USDT não encontrado no seu cofre",
};
